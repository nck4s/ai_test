...
## Запись видео через OBS Studio

Для записи видео выполнения тестов с помощью OBS Studio:

1. **Установите OBS Studio:**

   - Скачайте с официального сайта: https://obsproject.com/
   - Установите программу

2. **Настройте OBS для записи экрана:**

   - Откройте OBS Studio
   - В разделе "Источники" добавьте "Захват экрана" (Display Capture)
   - Выберите монитор, на котором будет отображаться браузер
   - Настройте разрешение и качество записи в настройках (Файл → Настройки → Вывод)

3. **Запустите запись перед запуском тестов:**

   - Нажмите кнопку "Начать запись" в OBS Studio
   - Запустите тесты в терминале: `pytest -q --headed`
   - После завершения тестов остановите запись

4. **Альтернативный способ (автоматическая запись):**
   - OBS Studio поддерживает автоматическую запись при запуске приложения
   - Настройте горячие клавиши для быстрого старта/остановки записи
